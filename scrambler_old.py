...
# matplotlib for embedding plots into Tk
...
# ====== FFMPEG HELPERS ======
def read_audio(path):
    """Decode any audio to float32 mono 44100 Hz numpy array (shape Nx1)."""
...
def write_audio(path, data, sr):
    """Save numpy -> audio using ffmpeg via a temporary WAV."""
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
        wav_path = tmp.name
...
def downsample_for_plot(y, max_points=200_000):
    ...
    # Обрезаем лишние элементы, чтобы reshape был корректным
...
def plot_wave_on_axis(ax, data, sr, title=""):
    ...
    ax.plot(t, y_ds)  # do not set colors/styles (per requirement)
...
class ScramblerApp:
    def __init__(self, root):
        self.keys = []
        self.root = root
        root.title("Аудио-скремблер 🎧 + графики")
        root.geometry("900x700")
        root.resizable(True, True)

        self.key_label = tk.Label(root, text="Ключ шифрования: —", fg="green")
        self.key_label.pack(anchor="w", padx=8, pady=4)
        ...
        tk.Checkbutton(ctl_frame, text="Разная перестановка для каждого окна", variable=self.varying).grid(row=4, column=0, columnspan=3, sticky="w")
    ...
    def generate_key(self):
        return (
            ...
            f"varying={self.varying.get()}; "
            f"unscramble={self.unscramble.get()}"
        )
    ...
    def play_result(self):
        if self.outfile and os.path.exists(self.outfile):
            ...
            try:
                ...
                # do not remove immediately because ffplay needs to read; leave cleanup to OS or user
                pass
        else:
    ...
    def process(self, path, outfile):
        try:
            ...
            if len(self.keys) == 1:
                key_str = f"Ключ (одно окно): {self.keys[0]}"
            else:
                key_str = "Ключи:\n" + "\n".join([f"Окно {i+1}: {k}" for i, k in enumerate(self.keys)])
            ...
            self.root.after(0, lambda: self.key_label.config(text="Ключ шифрования: " + key))
